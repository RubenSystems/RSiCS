...
enum FrameInsertResponse add_packet_to(struct FramePool * pool, struct Packet * packet) {
	...
	for (unsigned int i = 0; i < sizeof(pool->frames) / sizeof(struct Frame); i ++) {
		if (pool->frames[i].frame_id == packet->transmitable_data.uid) {
			...
			return (pool->frames[i].required_packets == ++pool->frames[i].recieved_packets) ? FRAME_COMPLETE : FRAME_INSERTED;
